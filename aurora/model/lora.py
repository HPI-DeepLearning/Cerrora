...
class LoRARollout(nn.Module):
    ...
    def forward(self, x: torch.Tensor, step: int) -> torch.Tensor:
        ...
        # Convert step to scalar if it's a tensor
        if isinstance(step, torch.Tensor):
            if step.numel() == 1:
                # Single element tensor
                step = step.item()
            else:
                # Multi-element tensor, take the first element or most common value
                # In practice, all elements in the batch should have the same rollout_step
                step = step[0].item()
